...
\title{%
  \textbf{Dissecting MLA Attention on H100:}\\
  \textbf{Reconstruction GEMM Bottlenecks and INT4 Weight Quantization}
}
...
% ══════════════════════════════════════════════════════════════════════════════
\begin{abstract}
We present a kernel-level performance study of attention in the SGLang LLM serving framework on NVIDIA H100 SXM5, with two contributions. \textbf{First}, we characterize FlashInfer (CUDA/Cutlass with Hopper TMA) vs.\ SGLang's Triton backend through Nsight Compute profiling: FlashInfer achieves 89\% of 3.35~TB/s HBM peak in decode vs.\ Triton's 80\%, and 552~TFLOPS (56\% peak) in prefill vs.\ 209~TFLOPS, with the gap driven by TMA hardware loads unavailable to the Triton compiler. \textbf{Second}, and more significantly, we identify and quantify a previously uncharacterized bottleneck in Multi-head Latent Attention (MLA): the \emph{reconstruction GEMMs} (weight-absorbed BMMs that recover full-dimensional K/V from compressed latents) consume \textbf{61\% of attention-layer time} at batch size 1 on DeepSeek-V3-scale architectures---exceeding the attention kernel itself. We show these BMMs are memory-bound at all practical batch sizes (arithmetic intensity 1--93 vs.\ crossover at 295) and demonstrate that \textbf{INT4 weight-only quantization of reconstruction matrices preserves model quality} ($\Delta$PPL = +0.051 on wikitext-2). However, a custom Triton W4A16 batched GEMM kernel achieves only 0.49$\times$ of cuBLAS FP16 performance---we identify L2 cache residency of the 16~MB reconstruction weights as the root cause, invalidating the HBM-bound roofline assumption. This establishes that realizing INT4 gains for MLA reconstruction requires either CUDA-native kernels, cross-layer fusion, or operating regimes where L2 contention forces HBM traffic.
\end{abstract}
...
We fill this gap with a controlled study on H100 SXM5 using Llama-3-8B (GQA) and DeepSeek-V2/V3 (MLA) attention configurations. Our contributions:

\begin{itemize}[leftmargin=*,topsep=0pt]
  \item A complete decode and prefill performance map across batch sizes (1--512) and sequence lengths (256--8192), with both backends profiled at identical tensor shapes.
  ...
  \item \textbf{Quantification of MLA reconstruction GEMM overhead}: the weight-absorbed BMMs that reconstruct full K/V from compressed latents consume 61\% of attention-layer time at bs=1, a bottleneck invisible to standard attention benchmarks.
  \item \textbf{INT4 quantization: quality-safe but kernel-limited}: selective INT4 of reconstruction weights adds only +0.051 PPL, but a custom Triton W4A16 kernel achieves 0.49$\times$ (not 3.9$\times$) due to L2 cache residency invalidating the HBM-bound roofline assumption.
\end{itemize}
...
\subsection{Measured Speedup: The L2 Cache Barrier}
...
\textbf{The INT4 kernel is 2$\times$ slower than cuBLAS FP16, not 3.9$\times$ faster.}  Three factors explain the gap between roofline prediction and measured performance:

\begin{enumerate}[leftmargin=*,topsep=0pt]
  \item \textbf{L2 cache residency}: The total reconstruction weight per BMM is $128 \times 128 \times 512 \times 2 = 16$~MB, which fits within H100's 50~MB L2 cache.  After first access, weights are served from L2 at $\sim$12~TB/s, not HBM at 3.35~TB/s.  The roofline model assumes HBM-bound operation, which is violated.

  \item \textbf{INT4 dequantization overhead}: Each packed byte requires bit masking, shifting, signed extension, and type conversion---adding compute that the roofline model does not account for.  The stride-2 activation loads required by the even/odd INT4 packing further reduce cache efficiency.

  \item \textbf{cuBLAS batched dispatch}: \texttt{torch.bmm} uses hardware-optimized batched GEMM scheduling on H100.  Our Triton kernel launches $H \times \lceil M/BM \rceil \times \lceil N/BN \rceil$ thread blocks, while cuBLAS uses fused warp-level batching.
\end{enumerate}

\textbf{However}, the INT4 kernel is 30$\times$ faster than a per-head FP16 loop (0.073~ms vs.\ 2.19~ms), demonstrating that the batched grid approach is effective---the bottleneck is competing with cuBLAS, not the batching strategy itself.

\textbf{Implication}: Realizing INT4 speedup for MLA reconstruction requires either (a) CUDA-native kernels with Hopper's INT8 tensor core instructions (no INT4 MMA exists), (b) fusing reconstruction across multiple layers to increase effective matrix sizes beyond L2 capacity, or (c) systems where L2 pressure from concurrent operations (FFN, attention) evicts reconstruction weights back to HBM.
...
\subsection{The L2 Cache Barrier for Small-Matrix INT4}

Our INT4 kernel results expose a fundamental limitation of weight quantization for small matrices: when weights fit in L2 cache, reducing their precision does not reduce memory traffic---the data is already served at L2 bandwidth ($\sim$12~TB/s), far above HBM peak.  MLA reconstruction weights total 16~MB per BMM, comfortably within H100's 50~MB L2.

This creates a paradox: MLA's KV compression \emph{enables} small reconstruction matrices, which then \emph{resist} further optimization via weight quantization.  In practice, L2 contention from concurrent operations (FFN, attention, other layers) may partially evict reconstruction weights, shifting the balance.  This suggests that INT4 reconstruction gains are deployment-dependent---significant in high-throughput multi-tenant serving with L2 pressure, but negligible in isolated single-request latency benchmarks.
...
FlashAttention~\cite{flashattention} and FlashAttention-2~\cite{flashattention2} introduced tiling-based exact attention with $O(N)$ memory. FlashInfer~\cite{flashinfer} extends this with Hopper-native TMA support.  Triton~\cite{triton} enables Python-level GPU kernel development.  SGLang~\cite{sglang} integrates both backends for LLM serving.  DeepSeek-V2~\cite{deepseekv2} introduced MLA; DeepSeek-V3~\cite{deepseekv3} scaled it to 671B parameters with 128 attention heads.

Volkov~\cite{volkov2010} demonstrated that occupancy is not always the primary performance factor, motivating our analysis of FlashInfer's register-heavy approach.  Weight-only quantization for LLM inference has been studied extensively~\cite{gptq,awq}, but prior work focuses on FFN and attention projection weights---not the reconstruction matrices specific to MLA architectures.  To our knowledge, no prior public work quantifies the reconstruction GEMM overhead in MLA or evaluates selective INT4 quantization of these weights.
