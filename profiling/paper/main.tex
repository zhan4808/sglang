...
\begin{itemize}[leftmargin=*,topsep=0pt]
  ...
  \item \textbf{INT4 quantization feasibility}: selective INT4 of reconstruction weights ($W^\text{kc}$, $W^\text{vc}$) adds only +0.051 wikitext-2 PPL while offering 3.9$\times$ bandwidth reduction---establishing a quality-safe optimization target for MLA serving.
\end{itemize}
...
At batch size 1 (latency-critical single-query serving), INT4 reconstruction offers \textbf{3.94$\times$ theoretical speedup}---reducing the 2.17~ms full-model reconstruction overhead to $\sim$0.55~ms.

\textbf{Note:} Realizing this speedup requires a custom fused INT4 batched-GEMM kernel.  Standard INT4 GEMM libraries (e.g., torchao's TinyGEMM) are optimized for large LLM linear layers ($\geq$4096 dims), not for batched small matrices ($128 \times 128 \times 512$).  The kernel design itself is a contribution we leave to future work.
...
\subsection{Selective Quantization as a Design Principle}

Our INT4 results reveal that reconstruction weights are \emph{uniquely quantization-friendly}: +0.051 PPL for selective INT4 vs.\ +6.057 for naive INT4 of all weights.  This suggests a general principle for MLA serving: \textbf{quantize reconstruction weights aggressively, quantize other weights conservatively}.  The 3.9$\times$ theoretical bandwidth reduction at bs=1 represents a larger latency improvement than any attention kernel optimization.
...
\begin{itemize}[leftmargin=*,topsep=0pt]
  ...
  \item INT4 speedups are theoretical (roofline-derived); actual speedup requires a custom fused INT4 batched-GEMM kernel not yet implemented.
...
Our most actionable finding is that \textbf{selective INT4 quantization of reconstruction weights ($W^\text{kc}$, $W^\text{vc}$) is both safe and high-impact}: +0.051 perplexity degradation with 3.9$\times$ theoretical bandwidth reduction.  This establishes a concrete optimization target for MLA serving systems.  The immediate next step is a fused INT4 batched-GEMM kernel for the specific reconstruction shapes ($H \times d_\text{nope} \times d_\text{lora}$), which we leave to future work.
